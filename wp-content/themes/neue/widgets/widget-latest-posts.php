...
add_action( 'widgets_init', 'vw_widgets_init_latest_posts' );
if ( ! function_exists( 'vw_widgets_init_latest_posts' ) ) {
	function vw_widgets_init_latest_posts() {
		register_widget( 'Vw_widget_latest_posts' );
	}
}

if ( ! class_exists( 'Vw_widget_latest_posts' ) ) {
	class Vw_widget_latest_posts extends WP_Widget {
		private $default = array(
			'title' => '',
			'cat_ids' => array( 0 ),
			'style' => 'large-small-1-col',
			'count' => '5',
			'order' => 'latest',
		);
		...
		public function __construct() {
			...
			parent::__construct(
		 		'vw_widget_latest_posts', // Base ID
		...
		function widget( $args, $instance ) {
			...
			unset( $title ); // Avoid double title
		...
		function form( $instance ) {
			...
			<!-- categories -->
			<p>
				<label for="<?php echo $this->get_field_id('cat_ids'); ?>">Categories:</label>
				<select class="widefat" multiple="multiple" id="<?php echo $this->get_field_id( 'cat_ids' ); ?>[]" name="<?php echo $this->get_field_name( 'cat_ids' ); ?>[]">
			...
			<!-- style -->
			<p>
				<label for="<?php echo $this->get_field_id('style'); ?>">Layout Style:</label>
				<select class="widefat" id="<?php echo $this->get_field_id('style'); ?>" name="<?php echo $this->get_field_name('style'); ?>">
					<option value="large-small-1-col" <?php selected( $style, 'large-small-1-col' ); ?>>Large &amp; Small</option>
					<option value="large-grid-1-col" <?php selected( $style, 'large-grid-1-col' ); ?>>Large Grid</option>
					<option value="medium-grid-1-col" <?php selected( $style, 'medium-grid-1-col' ); ?>>Medium Grid</option>
					<option value="small-grid-1-col" <?php selected( $style, 'small-grid-1-col' ); ?>>Small Grid</option>
					<option value="boxed-1-col" <?php selected( $style, 'boxed-1-col' ); ?>>Boxed</option>
					<option value="small-author" <?php selected( $style, 'small-author' ); ?>>Small Author</option>
