...
if ( ! class_exists( 'Vw_widget_author_list' ) ) {
	class Vw_widget_author_list extends WP_Widget {
			...
			foreach ( $authors as $author ) : ?>
				<li>
					<a href="<?php echo get_author_posts_url( $author->ID ); ?>" title="<?php printf( esc_attr__('Permalink to %s', 'envirra'), get_the_author_meta( 'display_name', $author->ID ) ); ?>" rel="bookmark">
						<?php echo get_avatar( get_the_author_meta( 'user_email', $author->ID ), VW_CONST_AVATAR_SIZE_LARGE, '', get_the_author_meta( 'display_name', $author->ID ) ); ?>
					</a>
				</li>
			<?php endforeach;
			echo '</ul>';
		...
		function form( $instance ) {
			...
			<!-- role -->
			<p>
				<label for="<?php echo $this->get_field_id('role'); ?>">Author Role:</label>
