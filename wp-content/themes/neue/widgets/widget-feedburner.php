	...
	class Vw_widget_feedburner extends WP_Widget {
		...
		function widget( $args, $instance ) {
			...
			<div class="vw-widget-feedburner-container">
				<h4 class="vw-widget-feedburner-text"><?php _e( 'Subscribe to our email newsletter.', 'envirra' ); ?></h4>
				<form action="http://feedburner.google.com/fb/a/mailverify" method="post" target="popupwindow" onsubmit="window.open('http://feedburner.google.com/fb/a/mailverify?uri=<?php echo $feedburner_id; ?>', 'popupwindow', 'scrollbars=yes,width=550,height=520');return true">
					<input class="vw-widget-feedburner-email" type="text" name="email" value="<?php _e( 'Enter your e-mail address' , 'envirra' ) ; ?>" onfocus="if (this.value == '<?php _e( 'Enter your e-mail address' , 'envirra' ) ; ?>') {this.value = '';}" onblur="if (this.value == '') {this.value = '<?php _e( 'Enter your e-mail address' , 'envirra' ) ; ?>';}">
					<input type="hidden" value="<?php echo esc_attr( $feedburner_id ); ?>" name="uri">
					<input type="hidden" name="loc" value="en_US">			
					<input class="feedburner-subscribe" type="submit" name="submit" value="<?php _e( 'Subscribe' , 'envirra' ) ; ?>"> 
				</form>
			</div>
			<?php
