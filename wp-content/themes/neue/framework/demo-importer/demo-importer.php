<?php
defined( 'VW_DEMO_IMPORT_PATH' ) || define( 'VW_DEMO_IMPORT_PATH', get_template_directory().'/framework/demo-importer/demo-content' );

if ( ! function_exists( 'vwdemo_import_custom_sidebars' ) ) {
	function vwdemo_import_custom_sidebars() {
		echo '<p>Import custom sidebars</p>';

		$file_path = VW_DEMO_IMPORT_PATH.'/custom_sidebars.txt';
...
if ( ! function_exists( 'vwdemo_setup_menu' ) ) {
	function vwdemo_setup_menu() {
		...
		$mobile_menu_slug = 'sample-mobile-menu';
		$bottom_menu_slug = 'sample-bottom-menu';
...
if ( ! function_exists( 'vwdemo_setup_homepage' ) ) {
	function vwdemo_setup_homepage() {
		$front_page = get_page_by_title( 'Sample Home 1' );
		if ( ! empty( $front_page ) ) {
