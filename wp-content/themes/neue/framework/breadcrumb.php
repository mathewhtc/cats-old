...
if ( ! function_exists( 'vw_the_breadcrumb' ) ) {
	function vw_the_breadcrumb() {
		$args = array(
			'singular_post_taxonomy'=> 'category',
		);

		$title = '<span class="vw-breadcrumb-title">' .__( 'YOU ARE HERE:', 'envirra' ) . '</span>';
		$separator = __( '<span class="vw-breadcrumb-separator">&raquo;</span>', 'envirra' );

		$items = vw_get_breadcrumb_items();
		
		$breadcrumbs = '<div class="vw-breadcrumb" xmlns:v="http://rdf.data-vocabulary.org/#">';
		$breadcrumbs .= $title;
...
if ( ! function_exists( 'vw_get_breadcrumb_items' ) ) {
	function vw_get_breadcrumb_items() {
		global $wp_query;
		...
		/* If viewing any type of archive. */
			...
			if ( is_category() || is_tag() || is_tax() ) {
				...
				if ( is_day() )
					$item['last'] = __( 'Archives for ', 'envirra' ) . get_the_time( 'F j, Y' );

				elseif ( is_month() )
					$item['last'] = __( 'Archives for ', 'envirra' ) . single_month_title( ' ', false );

				elseif ( is_year() )
					$item['last'] = __( 'Archives for ', 'envirra' ) . get_the_time( 'Y' );
			}

			else if ( is_author() )
				$item['last'] = __( 'Archives by: ', 'envirra' ) . get_the_author_meta( 'display_name', $wp_query->post->post_author );
		}

		/* If viewing search results. */
		else if ( is_search() )
			$item['last'] = __( 'Search results for "', 'envirra' ) . stripslashes( strip_tags( get_search_query() ) ) . '"';
