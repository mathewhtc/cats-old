...
if ( ! function_exists( 'vwspc_init_scripts' ) ) {
	function vwspc_init_scripts() {
		...
		wp_enqueue_style( 'vwspc-css-page-composer', get_template_directory_uri().'/framework/simple-page-composer/css/simple-page-composer.css' );

		wp_enqueue_script( 'vwjs-bootstrap-dropdown' );
		wp_enqueue_script( 'vwspc-js-page-composer', get_template_directory_uri().'/framework/simple-page-composer/js/simple-page-composer.js', array( 'jquery' ), VW_THEME_VERSION, true );
		...
		<div id="vwspc-container">
			...
			<script id="vwspc-template-section" type="text/template">
				<div class="vwspc-section">
					...
					<div class="vwspc-section-options hidden"></div>
				</div>
			</script>
			...
			<script id="vwspc-template-field-number" type="text/template">
				<input class="vwspc-field" type="number" name="quantity" min="1">
			</script>
...
if ( ! function_exists( 'the_simple_page_composer' ) ) {
	function the_simple_page_composer( $args=array() ) {
		$defaults = array(
			'before' => '',
			'after' => '',
			'before_section' => '<div class="vwspc-section vwspc-section-%s">',
			'after_section' => '</div>',
		);
